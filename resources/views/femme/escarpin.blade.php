...
@section('content')
<br>
<p style="font-size: 5vw;text-align:center;color:#5e5251;font-weight:bold ">Les escarpins</p>
...
<div class="row" style="margin-top:50px;">
        @foreach ($escarpins as $escarpin)
        <div style="display:inline-block;width:320px;margin:0 auto;" >
            <div class="card col-lg-3 col-md-4 col-sm-6 col-12   " >
                <img src={{asset ('./uploads/'.$escarpin["image"])}} alt="" class="card__img"> 
              <br>
                <!-- <div class="card__content">  -->
                     <div class="card__data">
                         <h1 class="card__title">{{$escarpin["titre"]}}</h1>
                         <span class="card__preci">{{$escarpin["prix"]}}  DH</span>
                         <p class="card__description">{{$escarpin["description"]}}</p>
                         @if(!Auth::user())
                         <a href="{{route('first')}}" class="card__button">Ajouter au panier</a>
                         @endif
                         @if(Auth::user() && Auth::user()->hasRole("utilisateur"))
                         <form action="{{route('addPanier',['titre' =>$escarpin["titre"],'prix' =>$escarpin["prix"],'image' =>$escarpin["image"],'id_user' =>Auth::user()->id ])}}" method="GET" >
            ...
            <div class="btn"><a href="{{route('escarpin.edit',$escarpin["id"])}}" class="btn btn-primary"> <img src="./uploads/modify.png" style="width:20px;margin-top:-5px"> Modifier</a></div>
            <form action="{{route('escarpin.delete',$escarpin->id)}}" method="POST">
              @csrf
              @method('DELETE')
              <div class="btn">
...
@section('style')
    <style>
        body{
            background-image:url("{{asset ('./uploads/escarpins.png')}}");
